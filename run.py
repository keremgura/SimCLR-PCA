...
parser.add_argument('-a', '--arch', metavar='ARCH', default='resnet18',
                    ...
                    help='model architecture: ' +
                         ' | '.join(model_names) +
                         ' (default: resnet18)')
parser.add_argument('-j', '--workers', default=12, type=int, metavar='N', # 12 normally
...
parser.add_argument("--extra_transforms", default = 0, type = int, choices = [1, 0], help = '2: all simclr augmentations, 1: only cropping and flipping, 0: no extra augmentations')
...
parser.add_argument('--min_crop_scale', default = 0.3, type = float, help = 'minimum scale for cropping if extra transformations are used')
...
parser.add_argument('--resize', default = 32, type = int, help = "Resizing applied to stl10 image data")
...
# Parameters for original SimCLR
...
def main():
    ...
    resize = args.resize if args.dataset_name != "cifar10" else 32
    transform_list = []
    ...
    model = ViTSimCLR(args) if args.vit else ResNetSimCLR(base_model=args.arch, out_dim=args.out_dim, dropout=args.dropout)
